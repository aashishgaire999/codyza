...
export default function AdminContentPage() {
  ...
  function loadNews(item: Item) {
    setSelectedNewsId(item.id)
    setNews({ slug: item.slug || "", title: item.title || "", summary: item.summary || "", body: item.body || "", tag: item.tag || "update", cover_image_url: item.cover_image_url || "", cover_image_alt: item.cover_image_alt || "", status: item.status || "draft" })
  }

  function loadAnnouncement(item: Item) {
    setSelectedAnnouncementId(item.id)
    setAnnouncement({ title: item.title || "", body: item.body || "", kind: item.kind || "general", meeting_url: item.meeting_url || "", starts_at: item.starts_at ? String(item.starts_at).slice(0, 16) : "", ends_at: item.ends_at ? String(item.ends_at).slice(0, 16) : "", published: Boolean(item.published) })
  }

  function resetNews() { setSelectedNewsId(null); setNews({ slug: "", title: "", summary: "", body: "", tag: "update", cover_image_url: "", cover_image_alt: "", status: "draft" }) }
  function resetAnnouncement() { setSelectedAnnouncementId(null); setAnnouncement({ title: "", body: "", kind: "general", meeting_url: "", starts_at: "", ends_at: "", published: false }) }
  ...
  async function updateMediaAlt(item: Item, alt: string) {
    setBusy(true); setError("")
    const response = await fetch("/api/admin/media", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: item.id, alt_text: alt }) })
    if (!response.ok) setError((await response.json()).error || "Could not update media")
    else await load()
    setBusy(false)
  }
  ...
  return (
    <div className="cosmic-workspace cosmic-admin min-h-screen text-foreground" data-cosmic-zone="command">
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        {active === "news" && <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]"><form onSubmit={(event) => { event.preventDefault(); void save("news", { ...news, ...(selectedNewsId ? { id: selectedNewsId } : {}) }) }} className="surface-card space-y-4 p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="member-hero-label">{selectedNewsId ? "editing saved story" : "new story"}</p><h2 className="text-lg font-semibold">{selectedNewsId ? news.title || "Untitled post" : "Write a news post"}</h2></div>{selectedNewsId && <button type="button" onClick={resetNews} className="text-xs text-muted-foreground hover:text-foreground">new post</button>}</div><div className="grid gap-3 sm:grid-cols-2"><input className="glass-input px-3 py-2" placeholder="Slug" value={news.slug} onChange={(e) => setNews({ ...news, slug: e.target.value })} /><input className="glass-input px-3 py-2" placeholder="Title" value={news.title} onChange={(e) => setNews({ ...news, title: e.target.value })} /></div><textarea className="glass-input w-full px-3 py-2" rows={2} placeholder="Summary" value={news.summary} onChange={(e) => setNews({ ...news, summary: e.target.value })} /><textarea className="glass-input w-full px-3 py-2 font-mono text-sm" rows={12} placeholder="Post body — blank lines create paragraphs" value={news.body} onChange={(e) => setNews({ ...news, body: e.target.value })} /><div className="grid gap-3 sm:grid-cols-2"><input className="glass-input px-3 py-2" placeholder="Cover image URL" value={news.cover_image_url} onChange={(e) => setNews({ ...news, cover_image_url: e.target.value })} /><input className="glass-input px-3 py-2" placeholder="Cover image alt text" value={news.cover_image_alt} onChange={(e) => setNews({ ...news, cover_image_alt: e.target.value })} /></div><div className="flex flex-wrap gap-3"><select className="glass-input px-3 py-2" value={news.tag} onChange={(e) => setNews({ ...news, tag: e.target.value })}><option>update</option><option>launch</option><option>announcement</option></select><select className="glass-input px-3 py-2" value={news.status} onChange={(e) => setNews({ ...news, status: e.target.value })}><option>draft</option><option>published</option><option>archived</option></select><button disabled={busy} className="btn-primary ml-auto rounded-full px-5 py-2">{busy ? "saving…" : selectedNewsId ? "overwrite post" : "save post"}</button></div></form><ItemList entity="news" items={library.news} remove={remove} titleKey="title" onSelect={loadNews} selectedId={selectedNewsId} /></div>}

        {active === "media" && <div className="grid gap-6 lg:grid-cols-[22rem_minmax(0,1fr)]"><form onSubmit={upload} className="surface-card h-fit space-y-4 p-5"><p className="member-hero-label">media library</p><h2 className="text-lg font-semibold">Upload a new image</h2><input name="file" type="file" accept="image/jpeg,image/png,image/webp,image/gif" required className="block w-full text-sm text-muted-foreground" /><input value={altText} onChange={(e) => setAltText(e.target.value)} className="glass-input w-full px-3 py-2" placeholder="Describe the image" /><button disabled={busy} className="btn-primary w-full gap-2 rounded-full px-4 py-2"><Upload className="h-4 w-4" />upload</button><p className="text-xs text-muted-foreground">Select an existing image to edit its alt text and copy its live URL.</p></form><div className="grid grid-cols-2 gap-3 md:grid-cols-3">{library.media.map((item) => <article key={item.id} onClick={() => setSelectedMediaId(item.id)} className={`surface-card cursor-pointer overflow-hidden ${selectedMediaId === item.id ? "ring-2 ring-accent" : ""}`}><img src={item.public_url} alt={item.alt_text} className="aspect-video w-full object-cover" /><div className="space-y-2 p-3"><input value={item.alt_text || ""} onChange={(e) => { const next = library.media.map((media) => media.id === item.id ? { ...media, alt_text: e.target.value } : media); setLibrary({ ...library, media: next }) }} className="glass-input w-full px-2 py-1.5 text-xs" placeholder="Alt text" /><button onClick={() => void updateMediaAlt(item, String(item.alt_text || ""))} className="w-full rounded-full border border-border px-2 py-1.5 text-xs">save description</button><div className="flex items-center gap-2"><input readOnly value={item.public_url} className="min-w-0 flex-1 bg-transparent text-[10px] text-muted-foreground" onFocus={(e) => e.currentTarget.select()} /><button onClick={() => void remove("media", item.id)} aria-label="Delete image"><Trash2 className="h-4 w-4 text-destructive" /></button></div></div></article>)}</div></div>}
        ...
        {active === "announcements" && <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]"><form onSubmit={(event) => { event.preventDefault(); void save("announcements", { ...announcement, ...(selectedAnnouncementId ? { id: selectedAnnouncementId } : {}), starts_at: announcement.starts_at || null, ends_at: announcement.ends_at || null, meeting_url: announcement.meeting_url || null }) }} className="surface-card space-y-4 p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="member-hero-label">{selectedAnnouncementId ? "editing saved announcement" : "new announcement"}</p><h2 className="text-lg font-semibold">{selectedAnnouncementId ? announcement.title || "Untitled announcement" : "Post a member announcement"}</h2></div>{selectedAnnouncementId && <button type="button" onClick={resetAnnouncement} className="text-xs text-muted-foreground hover:text-foreground">new announcement</button>}</div><input className="glass-input w-full px-3 py-2" placeholder="Title" value={announcement.title} onChange={(e) => setAnnouncement({ ...announcement, title: e.target.value })} /><textarea className="glass-input w-full px-3 py-2" rows={5} placeholder="Announcement" value={announcement.body} onChange={(e) => setAnnouncement({ ...announcement, body: e.target.value })} /><div className="grid gap-3 sm:grid-cols-2"><select className="glass-input px-3 py-2" value={announcement.kind} onChange={(e) => setAnnouncement({ ...announcement, kind: e.target.value })}><option>general</option><option>meeting</option><option>release</option><option>urgent</option></select><input className="glass-input px-3 py-2" placeholder="Meeting link" value={announcement.meeting_url} onChange={(e) => setAnnouncement({ ...announcement, meeting_url: e.target.value })} /><input type="datetime-local" className="glass-input px-3 py-2" value={announcement.starts_at} onChange={(e) => setAnnouncement({ ...announcement, starts_at: e.target.value })} /><input type="datetime-local" className="glass-input px-3 py-2" value={announcement.ends_at} onChange={(e) => setAnnouncement({ ...announcement, ends_at: e.target.value })} /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={announcement.published} onChange={(e) => setAnnouncement({ ...announcement, published: e.target.checked })} />Publish to members</label><button disabled={busy} className="btn-primary rounded-full px-5 py-2">{selectedAnnouncementId ? "overwrite announcement" : "save announcement"}</button></form><ItemList entity="announcements" items={library.announcements} remove={remove} titleKey="title" onSelect={loadAnnouncement} selectedId={selectedAnnouncementId} /></div>}
      </main>
    </div>
  )
}

function ItemList({ entity, items, remove, titleKey, onSelect, selectedId }: { entity: Entity; items: Item[]; remove: (entity: Entity, id: string) => Promise<void>; titleKey: string; onSelect?: (item: Item) => void; selectedId?: string | null }) {
  return <aside className="surface-card h-fit p-4"><h2 className="mb-3 text-sm font-semibold">Saved items</h2><div className="space-y-2">{items.length === 0 ? <p className="text-xs text-muted-foreground">Nothing here yet.</p> : items.map((item) => <div key={item.id} onClick={() => onSelect?.(item)} className={`flex cursor-pointer items-center gap-2 rounded-xl border px-3 py-2 ${selectedId === item.id ? "border-accent/50 bg-accent/10" : "border-border bg-muted/40"}`}><span className="min-w-0 flex-1 truncate text-sm">{String(item[titleKey] || item.file_name || item.id)}</span><button onClick={(event) => { event.stopPropagation(); void remove(entity, item.id) }} aria-label="Delete item"><Trash2 className="h-4 w-4 text-destructive" /></button></div>)}</div></aside>
}
