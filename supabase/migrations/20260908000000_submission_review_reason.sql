-- Admins had no way to tell a submitter why their project was approved or
-- rejected beyond a generic "approved!"/"not approved this time" message.
-- Also grants select on group_id and this new column to anon/authenticated --
-- group_id was added in an earlier migration but never granted, so the
-- member-facing pages (which query Supabase directly, not through a server
-- route) could never actually read it despite selecting it.

alter table public.submissions add column if not exists review_reason text;

grant select (group_id, review_reason) on public.submissions to anon, authenticated;

-- Adding a third parameter changes the function's signature, so
-- create-or-replace would just add a new overload and leave the old
-- 2-arg version callable and stale. Drop it explicitly first.
...
begin
  ...
  reason_note := case when p_reason is not null and length(trim(p_reason)) > 0 then ' Note: ' || trim(p_reason) else '' end;
  ...
  update submissions set status = p_status, review_reason = p_reason where id = reviewed.id;
  ...
  values (
    ...
    case when p_status = 'approved'
      then 'Your project "' || reviewed.project_name || '" was approved! +' || coalesce(reviewed.xp_earned, 0) || ' XP added.' || reason_note
      else 'Your project "' || reviewed.project_name || '" was not approved this time.' || reason_note
    end,
    '/member/projects'
  );
  ...
  if reviewed.group_id is not null then
    ...
    select gm.codyza_id,
      case when p_status = 'approved' then 'submission_approved' else 'submission_rejected' end,
      case when p_status = 'approved'
        then 'Your group project "' || reviewed.project_name || '" was approved! +' || coalesce(reviewed.xp_earned, 0) || ' XP added.' || reason_note
        else 'Your group project "' || reviewed.project_name || '" was not approved this time.' || reason_note
      end,
      '/member/groups'
...
revoke all on function public.admin_review_submission(uuid, text, text) from public, anon, authenticated;
grant execute on function public.admin_review_submission(uuid, text, text) to service_role;
